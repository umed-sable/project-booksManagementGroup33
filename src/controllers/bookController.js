...
const createBook = async function (req, res) {
  try {
    ...
    if (!ObjectId.isValid(userId)) { return res.status(400).send({ status: false, msg: 'userId is required' }) }
...
const getBooks = async function (req, res) {
  ...
  try {
    ...
    let availableBooks = await bookModel.find({ $and: [filters, { deleted: false }]}).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 }).sort({ title: 1 })
          if(availableBooks.length>0)
          {return res.status(200).send({ status: true, msg: "book  list", data: availableBooks})}
...
//getBooksByPath......................
  ...
  const getBooksByPath=async function(req,res){
    try{
       ...
       {return res.status(400).send({status:false,msg:"Valid bookId is required" })}
       
        let book= await bookModel.findById({_id:bookId,deleted:false})
    ...
    let data = req.body
      ...
      if (ifExist.deleted == false) {
          ...
          let updatedBook = await bookModel.findByIdAndUpdate({ _id: Id },
              {
                  $set: { title: newTitle, excerpt: newExcerpt, releasedAt: newDate, ISBN:isbn },
              },
              { new: true })
...
*/
        ...
        else
        {
            const deleteData = await bookModel.findOneAndUpdate({ _id: { $in: findBook } }, 
                              ...
                              { new: true }).select({ _id: 1, title: 1, deleted: 1, deletedAt: 1 })
